...
Essendo il numero dei dati troppo ridotto per essere interpolato con successo dalla curva corrispondente ($1.1$), consideriamo una interpolazione lineare di tipo $y = A + Bx$ dove   $y= \nu^2$ e $x = \tau$.
\\
Utilizziamo il metodo dei minimi quadrati, ottenendo:
...
Notiamo innanzitutto che la legge fisica, quando è linearizzata, è una funzione $y=mx$, che quindi deve passare l'origine. Si osservi infatti come $A$ per la corda B ha un valore trascurabile rispetto al valore delle $y$, e quindi può essere approssimato a zero. 
Questa discordanza è confermata anche nel calcolo del $\chi^2$ che per entrambe le funzioni della corda A da valori svariati ordini di grandezza superiori al numero delle misure. 
Siamo portati a rigettare queste misure. Consideriamo ora tutti le misure di tutte le armoniche per la corda A e interpoliamo i dati con funzioni lineari.
...
Sempre dall'equazione $1.1$, si deduce la proporzionalità inversa tra la frequenza $\nu$ e la lunghezza $l$. Abbiamo un sufficiente numero di dati per utilizzare la curva corrispondente. 
