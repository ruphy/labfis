...
\nu_n= \frac{vn}{2L}		
\end{equation}
 con $v$ velocità del suono nel mezzo e n=1,2,3.. 
\\

Verifichiamo che data una $\nu$ fondamentale a cui associamo il valore $n=1$, quelle successive saranno multipli interi della fondamentale. 
...
con $m$  coefficente angolare della retta. Per la 7.1 si ha che $m=\frac{v}{2L}$, da cui ricaviamo $v$. bisogna prima apportare una correzione alla formula scritta in precedenza. Per un tubo aperto reale la posizione di nodi e antinodi dipende dal diametro del tubo, pertanto apportiamo la seguente correzione:

$$ L+ 0.8d = n\frac{\lambda}{2} $$

con $d$ diametro del tubo (3.9 cm). 
Si ricava $$v=2(L+0.8d)m=351.994 m/s$$


Si può ricavare $v$ direttamente dalla 7.1( con la correzione), sostituendo a $\nu$ la frequenza fondamentale $\nu = 185$ Hz

$$ v = \frac{\nu2(L+0.8d)}{n} = 344.544 m/s$$ 
 
 Si verifica sperimentalmente che all'interno di un intervallo di ampiezza 10 Hz lo strumento non è in grado di distinguere ulteriori variazioni nella lettura delle frequenze. Se ne ricava che l'errore associato alle misure è di 5 Hz.  
\section{Analisi}
