...
L'angolo di deviazione minima ($\delta_{min}$) è il più piccolo angolo misurato facendo incidere un fascio di luce su una faccia del prisma (con inclinazione variabile) e misurando l'ampiezza dell'angolo rifratto.

In tabella sono riportati i valori trovati per ogni misurazione dell'angolo rifratto. Per aumentare il livello di precisione, abbiamo raccolto i dati relativi agli angoli di deviazione minima per ogni faccia del prisma a base triangolare, identificando ogni misurazione con il numero dello spigolo compreso tra le faccie colpite e il verso di percorrrenza della luce.

Per ovvie ragioni geometriche (è presente un solo angolo acuto), è stato impossibile effettuare misure riferite a vari vertici.
